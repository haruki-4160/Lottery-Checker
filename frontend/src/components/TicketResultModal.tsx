...
import { Trophy, Frown, X, CheckCircle2, ShieldCheck, Share2, ArrowRight, FileText, ExternalLink } from 'lucide-react';
...
export const TicketResultModal: React.FC<TicketResultModalProps> = ({
  ...
  return (
    <>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm animate-in fade-in duration-200">
        ...
        >
          ...
          <div className="p-6 space-y-5">
            ...
            {result.pdf_url && (
              <div className="p-3 rounded-xl bg-slate-950/80 border border-amber-500/30 flex items-center justify-between gap-3 text-xs">
                <div className="flex items-center gap-2">
                  <FileText className="w-4 h-4 text-amber-400 shrink-0" />
                  <div>
                    <span className="font-semibold text-slate-200 block">
                      {lang === 'en' ? 'Official Kerala Govt Gazette' : 'ഔദ്യോഗിക സർക്കാർ ഗസറ്റ് രേഖ'}
                    </span>
                ...
                <button
                  ...
                  className="px-3 py-1.5 rounded-lg bg-amber-500/15 hover:bg-amber-500/25 border border-amber-500/30 text-amber-300 font-bold flex items-center gap-1 transition-colors shrink-0"
                >
            ...
            <div className="bg-slate-800/60 p-3.5 rounded-xl border border-slate-700/70 text-xs text-slate-300 space-y-1.5">
              ...
              <p className="text-[11px] text-slate-400 italic">
                {lang === 'en'
                  ? '* Always cross-verify with the official Kerala Government Gazette before claim submission.'
      ...
      {/* Embedded PDF Modal when clicked */}
